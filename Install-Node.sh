...
if [ "$EUID" -ne 0 ]
        ...
        exit
else LINKTONODE=$(curl -G https://nodejs.org/dist/latest-v4.x/ | awk '{print $2}' | grep -P 'href=\"node-v4\.\d{1,}\.\d{1,}-linux-'$PICHIP'\.tar\.gz' | sed 's/href="//' | sed 's/<\/a>//' | sed 's/">.*//');
# curl -G https://nodejs.org/dist/latest-v4.x/ | awk '{print $2}' | grep -P 'href=\"node-v4\.\d{1,}\.\d{1,}-linux-armv7l\.tar\.gz' | sed 's/href="//' | sed 's/<\/a>//' | sed 's/">.*//'
NODEFOLDER=$(echo $LINKTONODE | sed 's/.tar.gz/\//');
#Shell script created by @audstanley
#Next, Creates directory for downloads, and downloads node 4.x
cd ~/ && mkdir tempNode && cd tempNode && wget https://nodejs.org/dist/latest-v4.x/$LINKTONODE;
tar -xzf $LINKTONODE;
